...
export default function Checkout() {
  ...
  // Check credit eligibility for ratib buyers
  ...
  // Determine delivery partner per store
    ...
    try {
      ...
      for (const storeId of storeIds) {
        ...
        const { data: order, error: orderError } = await supabase.from('orders').insert({
          ...
          delivery_partner_id: info?.partner?.id || null,
          total_inr: orderTotal,
          total_display: `₹${Math.round(orderTotal)}`,
  ...
  return (
    <div className="px-4 py-4 space-y-4">
      ...
      {storeDeliveryInfo.some(d => d.partner) && (
        <Card className="p-4">
          <h2 className="text-sm font-semibold text-foreground mb-3 flex items-center gap-1"><Truck className="h-4 w-4" /> Delivery</h2>
          {storeDeliveryInfo.map(d => d.partner && (
            <div key={d.storeId} className="flex items-center gap-2 p-2 rounded-lg bg-secondary mb-1">
              ...
              <div>
                <p className="text-xs font-medium text-foreground">Delivered by {d.partner.name}</p>
        ...
        <RadioGroup value={paymentMethod} onValueChange={setPaymentMethod} className="space-y-2">
          ...
          {isRatib && (
            <label className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-colors ${
              ...
              <span className="text-xl">🏦</span>
              <div>
                <p className={`text-sm font-medium ${creditEligible ? 'text-primary' : 'text-muted-foreground'}`}>Pay on Credit</p>
                <p className="text-xs text-muted-foreground">
                  {creditEligible ? 'You are eligible for credit' : 'Credit unavailable – no store within 1 km of your address'}
                </p>
              </div>
            </label>
          )}
      ...
      <Card className="p-4 space-y-2">
        ...
        {items.slice(0, 5).map(item => {
          ...
          return (
            <div key={item.storeProductId} className="flex justify-between text-xs">
              ...
              <span className="text-foreground">₹{Math.round(price)}</span>
            </div>
          );
        })}
        ...
        <div className="border-t pt-2 space-y-1">
          ...
          <div className="flex justify-between text-sm">
            ...
            <span className="text-foreground">₹{totalDeliveryFee}</span>
          </div>
      ...
      <Button onClick={handlePlaceOrder} disabled={placing || !selectedAddress} className="w-full h-12 rounded-xl text-base font-semibold">
        {placing ? 'Placing Order...' : `Place Order · ₹${Math.round(total)}`}
      </Button>
    </div>
  );
}
