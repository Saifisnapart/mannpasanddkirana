...
export default function ProductDetail() {
  ...
  const { addItem, items, updateQty, updateCustomQty, currentStoreId, clearCart } = useCart();
  ...
  const handleAdd = (sp: any) => {
    ...
    if (currentStoreId && currentStoreId !== sp.store_id) {
      setPendingItem(sp);
  ...
  // Find current store name for conflict modal
  const currentStoreName = currentStoreId
    ? storeProducts?.find((sp: any) => sp.store_id === currentStoreId)?.stores?.name || 'Current Store'
    : '';
  const newStoreName = pendingItem?.stores?.name || 'New Store';
  ...
  return (
    <div className="px-4 py-4 space-y-5">
      ...
      <div>
        ...
        <p className="mt-1 text-lg font-bold text-foreground">{formatPrice(product.base_price_inr)}</p>
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-display text-base font-bold text-foreground">
            {t('sold_by')} ({sortedStoreProducts.length})
          </h2>
          <div className="flex gap-1">
            <Button size="sm" variant={sortBy === 'price' ? 'default' : 'outline'} onClick={() => setSortBy('price')} className="text-xs h-7 rounded-lg">{t('cheapest_first')}</Button>
            <Button size="sm" variant={sortBy === 'stock' ? 'default' : 'outline'} onClick={() => setSortBy('stock')} className="text-xs h-7 rounded-lg">{t('most_stock')}</Button>
          </div>
        </div>
        <div className="space-y-2">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {idx === 0 && <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-primary/10 text-primary font-medium">{t('cheapest_first')}</span>}
                    </div>
                    <div className="flex items-center gap-2 text-xs text-muted-foreground mt-0.5">
                      {sp.distance !== null && <span className="flex items-center gap-0.5"><MapPin className="h-3 w-3" />{sp.distance} km</span>}
                      <span>{t('stock')}: {sp.stock_quantity}</span>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-lg font-bold text-foreground">{formatPrice(displayPrice)}</p>
                    <p className="text-[10px] text-muted-foreground">{formatPrice(sp.vendor_price_inr)}/{product.base_unit}</p>
                  </div>
                </div>
                <div className="mt-2">
                  {cartItem ? (
                    <div className="flex items-center gap-2">
                      ...
                      <Button onClick={() => navigate('/cart')} className="flex-1 rounded-xl h-9 text-sm">{t('cart')}</Button>
                    </div>
                  ) : (
                    <Button onClick={() => handleAdd(sp)} disabled={sp.stock_quantity === 0} className="w-full rounded-xl h-9 text-sm font-semibold">
                      {t('add_to_cart')} · {formatPrice(displayPrice)}
                    </Button>
                  )}
                </div>
              </Card>
      ...
      <CartConflictModal
        ...
        currentVendor={currentStoreName}
        newVendor={newStoreName}
